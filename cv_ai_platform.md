# Samuel Toyoshi Ishida — AI Platform Engineer

## Contact
- **Email:** samuel.toyoshi@gmail.com
- **LinkedIn:** linkedin.com/in/samuelishida
- **Location:** São Paulo, Brazil

## Professional Summary

8+ years of software development, 3+ years hands-on AI/ML. Expert in RAG systems, LLM orchestration, and AI tooling for developer productivity. Python, TypeScript, Java. AWS, Azure, DevOps (Docker, Terraform, CI/CD). At Goldman Sachs: building AI platforms for FICC trading — combining production SRE with retrieval-augmented systems and automated reporting.

## Technical Skills

**AI / ML:** LLM orchestration (OpenAI, Anthropic, local), RAG, vector databases (Chroma, Pinecone), embeddings, prompt engineering, AI documentation, LangChain, agentic workflows

**Languages & Frameworks:** Python, TypeScript, Java, Node.js, Django/Flask, FastAPI, Spring Boot, REST APIs, gRPC

**MLOps & Infrastructure:** AWS (SageMaker, Bedrock, Lambda), Azure, Docker, Kubernetes, Terraform, Git/GitHub, CI/CD, ML pipeline automation

**Data & Observability:** PostgreSQL, MongoDB, Redis, Apache Kafka, Prometheus, Grafana, structured logging, model performance monitoring

## Professional Experience

### Goldman Sachs — DevOps Engineer | AI Platform Focus
*Oct 2024 – Present | São Paulo, Brazil*

- Built a production support AI agent leveraging RAG and GitHub Copilot.
- Built AI-assisted docs tool with RAG, using OpenAI's API running in a VPS.
- Migrated internal Apache servers from RHEL 7 to RHEL 8, producing documentation for other clusters to follow
- Collaborated with the Strats team to optimize infrastructure used for risk curves and pricing models.
- Delivered trainings on GitHub Copilot and AI-assisted workflows.

### Alctel Telecom — DevOps Engineer
*May 2023 – Mar 2024 | Belo Horizonte, Brazil (remote)*

- Gathered client business requirements and integrated chatbot solutions with WhatsApp using C#, .NET and MySQL.
- Provided technical support to resolve production issues.

### GFT Group — DevOps Engineer
*Jan 2022 – Mar 2023 | São Paulo, Brazil (remote)*

- Led the migration of 200 on-premise applications to AWS using CloudFormation.
- Collaborated with the international team to automate document generation using Terraform and Python.

### Suma – Consulting Group — Data Engineer
*Nov 2019 – Jan 2022 | Campinas, São Paulo*

- Built ETL pipelines to ingest and preprocess large scale sensor data, preparing datasets for route optimization.
- Integrated low-latency UDP communication protocols from custom-built telemetry sensors with SUMA servers.
- Managed a cloud data warehouse on Azure, engineering pipelines and integrating with PowerBI for AI-driven insights via Power BI.

### Bit Capital — Backend Engineer
*Jul 2019 – Oct 2019 | Campinas, Brazil*

- Developed high performance APIs in TypeScript to be used in credit cards transacions.
- Worked in the integration with Pix system

### Boilesen Associates — Full-Stack Developer
*Mar 2019 – Jul 2019 | Campinas, Brazil*

- Developed a digital marketing tool for lead generation and tracking using PHP and the Slim framework.
- Provided client support for Android Java applications, enhancing user experience and satisfaction.

### Stoom — Software Engineer Intern
*Jan 2018 – Oct 2018 | Campinas, Brazil*

- Developed the Suprevida website using Apache Struts 2 and PostgreSQL.
- Collaborated with the team to address client demands and gather business requirements.
- Assisted in feature development and issue resolution for Brazil's leading retailer of domestic animal products.

### Devnup IT Solutions — Software Engineer Intern
*Jul 2017 – Dec 2017 · 6 mos*

- Developed a sports distribution platform, Matchup Sports, utilizing Play, Spring, and AngularJS at Devnup IT Solutions.
- Enhanced platform functionality and user experience through skills in Springboot, Git, Docker, Java, and AngularJS.

## Education

- **B.Tech Information Security** — Estácio (Feb 2023 – Nov 2025)
- **BS Computer Engineering** — UNICAMP (2014 – Feb 2019)

## Certifications & Highlights

- **Foundations of Cybersecurity** — Google
- **Cambridge B2 First** — Score 169
- **OBI — Silver Medal** (Brazilian Olympiad in Informatics)
- **UNICAMP Phoenix Robotics Team** (2014–2016): developed autonomous robot with visual cone recognition using OpenCV and Python

## Languages

- **Portuguese:** Native
- **English:** Full Professional (Cambridge B2, Score 169)